...
public class AVLTree < E
  ...
  private class AVLListIter implements Iterator<E> {

    private Stack<Node<E>> iteratorStack;
    private Node<E> current;
    private Node<E> lastItemReturned;

    public AVLListIter() {
      // Initializing stack
      iteratorStack = new Stack<Node<E>>();
      current=null;
      lastItemReturned=null;
      Node<E> curr = root;

      /**
       * Goes left until reached the leaf node
       */
      while (curr != null)
      {
        iteratorStack.push(curr);
        curr = curr.left;
      }
    }
    ...
    /**
     *It checks if the Stack if empty or not
     */
    @Override
    ...
    public E next() {
      ...
      {
        current=current();
        lastItemReturned=current;
        next2();
        return lastItemReturned.data;
      }
      System.out.println("You reached the last element of AVL Tree");
      return null;
    }

    /**
     * Helper Method of next method
     */
    public void next2(){
      Node<E> temp = iteratorStack.peek().right;
      iteratorStack.pop();
  ...
  AVLListIter iter; //iterator object of AVLTree

  /**
   * This method constructs AVLIterator with delegation
   */
  public void iteratorConstructor(){
    iter=new AVLListIter();
  }

  /**
   * This method calls next method of my iterator with delegation
   * @return
   */
  public E next(){
    return iter.next();
  ...
  /**
   * I used iterator i have built. Thanks to this iterator
   * The method can traverse hole tree in sequential order
   * //if item is bigger than the data, offer it into the Queue
   * @param item
   */

  public void tailSet(E item){
    iteratorConstructor();
    Queue<E> tempQueue= new LinkedList<E>();
    E temp;
    while(iter.hasNext()){
      temp=iter.next();
      if(item.compareTo(temp)<1) //if item is bigger than the data, offer it into the Queue
        tempQueue.offer(temp);
    }
    while(!tempQueue.isEmpty()){
      System.out.print(tempQueue.poll()+",");
    }
    System.out.println(" are greater or equal to "+item);
  }

  /**
   * This method used the iterator that i have written. Iterator goes through the Tree in sequential
   * if item is equal or smaller than the data, offer it into the Queue
   * @param item
   */

  public void headSet(E item){
    iteratorConstructor();
    Queue<E> tempQueue= new LinkedList<E>();
    E temp;
    while(iter.hasNext()){
      temp=iter.next();
      if(item.compareTo(temp)>0) //if item is equal or smaller than the data, offer it into the Queue
        tempQueue.offer(temp);
    }
    while(!tempQueue.isEmpty()){
      System.out.print(tempQueue.poll()+",");
    }
    System.out.println(" are smaller than "+item);
    iteratorConstructor();
  }
